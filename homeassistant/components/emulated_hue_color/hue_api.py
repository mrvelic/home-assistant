...
from homeassistant.components.light import (
    ATTR_BRIGHTNESS, SUPPORT_BRIGHTNESS, ATTR_COLOR_XY, ATTR_COLOR_RGB, 
    SUPPORT_COLOR, ATTR_COLOR_TEMP, SUPPORT_COLOR_TEMP
)
...
def parse_hue_api_put_light_body(request_json, entity):
    ...
    if entity.domain == "light":
        ...
        elif HUE_API_STATE_CT in request_json:

        elif HUE_API_STATE_HUE in request_json \
            or HUE_API_STATE_SAT in request_json:


    return (result, brightness, colormode, color_xy, color_temp, hue, sat)
...
def entity_to_json(config, entity, is_on=None, brightness=None, colortemp=None, 
    colormode=None, xy=None, hue=None, sat=None):

    """Convert an entity to its Hue bridge JSON representation."""
    light_type = HUE_API_DEVICE_TYPE_DIMMABLE
    entity_features = entity.attributes.get(ATTR_SUPPORTED_FEATURES, 0)

    if entity.domain == "light":
        if entity_features & SUPPORT_COLOR:
            light_type = HUE_API_DEVICE_TYPE_COLOR
            if entity_features & SUPPORT_COLOR_TEMP:
                light_type = HUE_API_DEVICE_TYPE_EXTENDED_COLOR
        elif: entity_features & SUPPORT_COLOR_TEMP:
            light_type = HUE_API_DEVICE_TYPE_COLOR_TEMP
